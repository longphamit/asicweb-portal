...
export async function GET(req, { params }) {
  ...
  const account = await getDocumentById("activity", id);
  if (!account) {
    return new Response(JSON.stringify({ message: "Không tìm thấy hoạt động" }), { status: 404 });
  }
...
export async function DELETE(req, { params }) {
  ...
  const result = await deleteDocumentById("activity", id);
