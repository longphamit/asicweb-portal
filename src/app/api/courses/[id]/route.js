...
export async function GET(req, { params }) {
  ...
  const account = await getDocumentById("course", id);
  if (!account) {
    return new Response(JSON.stringify({ message: "Không tìm thấy khóa học" }), { status: 404 });
  }
...
export async function DELETE(req, { params }) {
  ...
  const result = await deleteDocumentById("course", id);
