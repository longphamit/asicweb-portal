...
export default function CoursesPage() {
  ...
  const limit = 100 // Số lượng bản ghi trên mỗi trang

  useEffect(() => {
    fetchCourses()
  }, [currentPage]) // Fetch lại khi đổi trang

  const fetchCourses = async () => {
    setIsLoading(true)
    try {
      // Gửi tham số page và limit lên API
      ...
      // Giả sử API trả về dạng { data: [...], totalPages: 5 }
      const coursesArray = Array.isArray(data) ? data : data.data || data.courses || []
      setCourses(coursesArray)
      
      // Cập nhật tổng số trang từ API nếu có, nếu không mặc định là 1
